...
import cht.model.HotelUpdateService;

@WebServlet( urlPatterns={"/HotelUpdateServlet.controller"} )
public class HotelUpdateServlet extends HttpServlet  {
	...
	public void init() throws ServletException {
		...
		hotelUpdateService = (HotelUpdateService) context.getBean("hotelUpdateService");
	}
	...
	protected void doGet(HttpServletRequest request,
		...
		if(hotelid!=0){
			reviewAvg = hotelUpdateService.getReviewAvg(bean);
			if(reviewAvg!=null){
		...
		if("Insert".equals(hotelaction)) {
			Hotel result = hotelUpdateService.insert(bean);
			if(result==null) {
		 ...
		 }else if("Update".equals(hotelaction)) {
			Hotel result = hotelUpdateService.update(bean);
			if(result==null) {
